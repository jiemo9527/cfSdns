...
def get_cf_ips() -> Tuple[List[str], List[str], List[str]]:
    ...
    # 限制每个列表最多50个
    final_ct_ip = final_ct_ip[-50:]
    final_cm_ip = final_cm_ip[-50:]
    final_cu_ip = final_cu_ip[-50:]
    ...
    print(f"电信IP: 剔除 {len(final_ct_ip) - len(final_ct_ip_filtered)} 个, 剩余 {len(final_ct_ip_filtered)} 个。")
    print(f"移动IP: 剔除 {len(final_cm_ip) - len(final_cm_ip_filtered)} 个, 剩余 {len(final_cm_ip_filtered)} 个。")
    print(f"联通IP: 剔除 {len(final_cu_ip) - len(final_cu_ip_filtered)} 个, 剩余 {len(final_cu_ip_filtered)} 个。")
